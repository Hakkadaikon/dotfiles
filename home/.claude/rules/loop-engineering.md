...
0. **抽出ループ(仕様 → 要件の網羅)**: EARS を書く前に、元仕様(RFC、標準、自然言語仕様)から要件を漏れなく抜く。3ループは「与えられた要件」を厳密に検証するが、抽出の網羅は守らない。漏れは入口に落ちる。原則は **過剰抽出は安全、漏れは危険**(採り過ぎは後で背景に落とせる、採り損ねは下流に現れない)。迷ったら採る。
   - **入口**: 仕様を全件走査し要件候補を出し切る。走査アンカーは文書型で変える(規範語=RFC 2119 はその一例、ほかに日本語規範表現「〜しなければならない」、OpenAPI/protobuf の全要素列挙、ストーリーの受け入れ条件など)。頭の中で走査せず、**採番チェックリストで消し込む**: 元仕様を `tasks/loopeng/` にコピー → 各単位に連番 ID(`S-001`...)+ チェックボックス → 各 ID を EARS へ変換して `[x]` → 未チェック・欠番が無いことを確認して閉じる。ID はそのままトレーサビリティ表の「仕様条項」列になる。

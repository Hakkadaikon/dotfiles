...
**使う(IaC の動的側面)**: Terraform / Pulumi / SST / CloudFormation などの IaC は厳密であってほしいので、**インフラ状態の遷移・依存順序・並行性が絡む部分**は積極的にこのループで固める。具体的には次が状態機械/並行性を持つ:
- リソースの **create / update / replace / destroy** ライフサイクルと、`create_before_destroy` 等の置換順序。
- リソース間の **依存グラフと apply 順序**(順序を誤ると参照未解決や一時的な権限欠落が起きる)。
- **並行 apply / state lock** の競合(複数パイプラインや並列リソース適用の interleaving)。
- **apply 途中失敗からのロールバック/再 apply** で状態が壊れない不変条件(部分適用後の整合性、冪等性、drift 後の収束)。
- ステートマシン的なインフラそのもの(Step Functions、デプロイの blue/green 切替、フェイルオーバ手順)。

これらは「desired state を1回宣言すれば終わり」ではなく、**遷移の途中状態と順序と並行**があるので TLA+ の網羅探索が効く。0段では IaC コード/モジュール仕様を元仕様として、上記の遷移・順序・並行・失敗時の不変条件を要件として抽出する。

**使わない(こちらが多数派)**: 通常の CRUD・UI・ビジネスロジック、状態遷移も並行性も無い逐次処理、時間制約の厳しいタスク。これらは通常の TDD で足りる。**TLA+ が価値を持つ場面はそもそも稀**だと自覚する。
- **IaC でも静的な宣言部分は対象外**: 単発リソースの宣言、命名規約、タグ/必須フィールドの有無、ポリシーの静的妥当性などは状態遷移が無い。これらは `terraform validate` / `tflint` / OPA・Conftest / 各社の policy-as-code / `terrashark` スキルなど**静的検査の領分**で、TLA+ を持ち込むのは過剰。動的な遷移・順序・並行が無いなら使わない。

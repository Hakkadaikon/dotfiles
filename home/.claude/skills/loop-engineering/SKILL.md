...
上の表で「使う」に入っても、次を自問する。引っかかったら**通常のテストへ逃がす**。

1. **動的(状態遷移・並行・順序)が本当にあるか?** 表の左列に当たるか。無ければやめる。
2. **状態空間が有限化できそうか?** カウンタや無制限コレクションで爆発しそうなら TLC は即死する。`CONSTANT` で値域を絞れる範囲で抽象化できるときだけ TLA+。無理なら property-based testing に落とす。

ループ自体は AI が裏方で回し、台帳・spec・hook はリポジトリに含めない(「成果物の置き場」参照)。だから導入・学習コストは判断材料に入れない。**動的があり有限化できるなら回す。**

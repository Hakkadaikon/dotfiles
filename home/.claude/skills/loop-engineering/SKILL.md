...
**この0段は採番チェックリスト方式(下記「入口」の手順)を必須ゲートとする。** ループを回すたびに毎回 `tasks/loopeng/<Name>.extract.md` の抽出台帳を作り、全 ID が `[x]`・連番に欠番なしになるまで外ループ(EARS)へ進まない。台帳を省いて頭の中で抽出する、または既存の EARS から逆に台帳を埋めた体にする、のは禁止。台帳が無い/未完なら設計検証は未着手とみなす。元仕様が口頭の要求だけで原文ファイルが無いときも、その要求文を `tasks/loopeng/` に書き起こして台帳の起点にする。
...
1. **入口**: 仕様を全件走査して要件候補を出し切る。目的は「仕様の各単位を1つも飛ばさず要件候補に変換する」こと。手段は文書の型で変わる。**「規範語 grep」は数あるアンカーの一つに過ぎず、RFC 形式の仕様にしか効かない**。まず仕様の構造単位(走査アンカー)を選ぶ:
   ...
   **採番チェックリスト方式(走査を機械的に詰める実務手順)**: 上の「全件走査」を頭の中でやると漏れる。**消し込み可能なワークシート**に落として潰す。1つの作業ファイルで完結させ、別形式への変換を増やして源泉を分裂させない。
...
2. **工程: トレーサビリティ・マトリクス(漏れの本命対策)**: 抽出した各規範文に一意 ID を振り、**「仕様条項ID → 要件(EARS R番号) → 形式手法(TLA+ INV / Lean P番号) → テスト」を1行で対応づける表**を作る。漏れが「空欄」という見える形になるのが肝。
   ...
   **この1枚を全ループ共通の消し込み台帳にする(新しいチェックリストを各ループで増やさない)。** 採番チェックリストの「全単位に ID → 消し込み」という発想は0段専用ではなく各ループで効くが、ループごとに別台帳を作ると源泉が分裂する。**このマトリクスの各列が各ループの消し込み対象**だと捉え、列が全行埋まったかでそのループの網羅を判定する:

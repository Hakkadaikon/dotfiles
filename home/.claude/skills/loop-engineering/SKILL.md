...
- 作業場所は `tasks/loopeng/` 配下(`tasks/todo.md` と同じエリア)。`.tla`/`.cfg`/`.feature`・トレーサビリティ表・TLC のスクラッチ(`states/` 等)はここで作り込む。リポジトリの `.gitignore` 状況に関わらず、`git add` の対象にしない(スクラッチは従来どおり随時消す)。

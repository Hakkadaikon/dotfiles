...
- **負荷テスト(Load)**：想定される通常〜ピークの負荷をかけ、その範囲で性能が劣化しないことを確認する。
...
- **スケーラビリティテスト**：負荷やデータ量の増加に対し、リソース追加で性能が伸びるか(水平/垂直)を確認する。
...
正常入力集合(ポジティブ)と不正・極値入力集合(ネガティブ・境界外)を同値分割と境界値で出し切り、すべて覆う。

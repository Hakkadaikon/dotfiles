...
### 網羅の定義
- **網羅基準（いつ網羅完了とみなすか）**: 各判定（if/while/switch/三項）の真分岐・偽分岐を両方とも最低1回ずつ通したとき（C1=100%）。
- **網羅手順（基準を満たすケース集合の作り方）**:
  1. コード中の判定をすべて列挙し、各判定に真・偽の2マスを割り当てる。
...
- **達成チェック（漏れの検出）**: branch coverage が100%で、switch default の未踏マスが残っていないこと。
  ただし複合条件の各項がどう効いたかは C1 では測れないので、そこは条件網羅・MC/DC に委ねる。
...
### 網羅の定義
- **網羅基準（いつ網羅完了とみなすか）**: 判定内の各 atomic condition が真・偽の両値を取ったとき。
- **網羅手順（基準を満たすケース集合の作り方）**:
  1. 複合条件を `&&` `||` で atomic condition に分解し、条件ごとに真・偽の2マスを並べる。
  2. 各条件の真・偽を踏む入力を選び、全条件のマスを埋めるようケースを組む。
  3. 短絡評価で評価到達しない条件が無いか確認し、到達する入力に直す。
- **達成チェック（漏れの検出）**: 真・偽の未割当マスが残る条件が無いこと。
...
### 網羅の定義
- **網羅基準（いつ網羅完了とみなすか）**: 理想は全実行可能経路だが、経路数は分岐に指数・ループで無限になり完全網羅は非現実的。
...
- **網羅基準（いつ網羅完了とみなすか）**: 理想は全 def-use ペアを通す（all-uses）こと。
  ただし完全網羅はパス網羅に近づき高コストなので、実務は重要変数（金額・権限フラグ）の def-use ペアに絞って各1経路を通したとき。
- **網羅手順（基準を満たすケース集合の作り方）**:
...
### 網羅の定義
- **網羅基準（いつ網羅完了とみなすか）**: これは構造網羅ではなくテスト強度のメタ基準。
  Mutation Score = Killed / (生成ミュータント − 等価ミュータント) を目標値（critical なモジュールでは高め）に達したとき。
- **網羅手順（基準を満たすケース集合の作り方）**:
  1. Stryker でミュータントを生成し、既存テストで Killed / Survived / No coverage を判定する。
  2. Survived には `toBe` 等の厳密アサートを足して値を固定し、kill する。
  3. No coverage はまず C0/C1 を埋め、再実行してスコアを上げる。
- **達成チェック（漏れの検出）**: Survived がアサーション不足、No coverage がカバレッジ不足の証拠として残っていないこと。
  等価ミュータントは原理的に kill 不能なのでスコア100%を強要しない。計算コストが高いため差分や critical に限定する。

...
- **網羅基準(いつ網羅完了とみなすか)**: 対象ユースケースの主成功フローを1ケース、加えて全拡張/例外フローを各1ケース踏んだとき。
...
- **達成チェック(漏れの検出)**: 列挙したフローのうちケース未割当の例外/代替が残っていないか(価値は例外側にある)。各ケースの粒度が単体テストに落ちすぎていないか(フローの完結を見ているか)。
...
- **網羅基準(いつ網羅完了とみなすか)**: 想定する現実利用シナリオ集合(実データ・実ログ由来)を各1本ずつ通したとき。
- **網羅手順(基準を満たすケース集合の作り方)**:
  1. 複数ユースケースをまたぐ現実の経路を実ログ・実利用データから抽出する。
...
### 「網羅」の扱い(網羅は主張しない)
- **なぜ網羅を定義できないか**: 経験・直感由来で狙うため、入力空間を形式的に覆えない。
...
- **停止規範(どこまでやれば十分とするか)**: チャーター(範囲宣言)+ タイムボックス + セッション数で打ち切り、セッションノートを成果物とする。
...
- **停止規範(どこまでやれば十分とするか)**: スモーク / デモ前の短時間確認に用途を限定し、「網羅も合否基準も無い」ことを常に明示する。
